...
public class Passenger implements PassengerEventReceiver, Comparable<Passenger> {
    ...
    /**
     * Occurs when the passenger decides not to exit because the alarm has cleared.  
     * 
     * Deductions:
     * none
     * 
     ...
     * no termination condition because this is not a looping action
     */
    private class OverweightExitAbortAction extends DoorAction {

        public OverweightExitAbortAction(SimTime offset) {
            super(offset);
        }
        ...
        public void execute() {
            ...
            isQueued = false;
            //return to the car
            log("Returned to the car after alarm clears  ", info.endFloor, ",", info.endHallway);
            state = State.WAITING_IN_CAR;
            //move to the end of the line
            passengerHandler.requeue(Passenger.this);
            //restart with hall door checking
        ...
        @Override
        public String toString() {
            return "OverweightExitAbortAction";
        }
    }
     ...
     * passenger exits (except when another passenger exist and clears overweight)
     */
    private class OverweightExitStartAction extends DoorAction {
        ...
        public void execute() {
            ...
            /*
             * if the alarm has stopped ringing (because another passenger has exited)
             * begin overweight abort action
             *
             * return to exit the OverweightExitStartAction loop
             */ 
             
            if (!pc.carWeightAlarm.isRinging()) {
                log("Returned to the car after alarm clears at ", info.endFloor);
                doorAction.set(new OverweightExitAbortAction(info.doorBackoutDelay));
                return;
            }
    ...
    private class CarCallCheckAction extends CallAction {
        ...
        public void execute() {
            if (state != State.WAITING_IN_CAR) {
                return; //do nothing if we're not waiting in the car
            }
